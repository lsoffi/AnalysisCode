...
process.hltLevel1GTSeed.L1SeedsLogicalExpression = cms.string(
    'L1_ETM30 OR L1_ETM40 OR L1_ETM50 OR L1_ETM60 OR L1_ETM70 OR L1_ETM100'
    )
...
if myTest:
    ...
    if (isMC):
        ...
        process.treePath = cms.Path(process.hltLevel1GTSeed +                   process.goodVertices + process.metfilter + process.tree)
else :
    if (isMC):
        process.treePath = cms.Path(process.hltLevel1GTSeed + process.gentree + process.goodVertices                     + process.tree)
    else :
        process.treePath = cms.Path(process.hltLevel1GTSeed +                   process.goodVertices                     + process.tree)
